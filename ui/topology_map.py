...
class TopologyMap(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            f"background-color: {config.COLOR_PLOT_BG}; border: 1px solid {config.COLOR_BORDER};"
        )
        self._nodes = {}
    ...
    def add_device(self, ip: str, os_name: str, is_gateway: bool = False):
        ...
        # Color coding based on OS type
        if is_gateway:
            color = QColor("#FFD700")            # Gold — always universal
        elif "Windows" in os_name:
            color = QColor(config.COLOR_ACCENT_CYAN)
        elif "Linux" in os_name or "Android" in os_name:
            color = QColor(config.COLOR_ACCENT_GREEN)
        elif "Network" in os_name:
            color = QColor(config.COLOR_ACCENT_ORANGE)
        else:
            color = QColor(config.COLOR_TEXT_DIM)
        ...
        if is_gateway:
            ...
            if "Gateway" in self._nodes:
                ...
                line.setPen(QPen(QColor(config.COLOR_BORDER), 2))
